...
export const CustomersSales = () => {
  ...
  return <Box className="screen-content">
    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><Typography variant="h4" gutterBottom>Customers & Sales</Typography><Box sx={{ display: "flex", gap: 1 }}><Button component="a" href={`${process.env.REACT_APP_API_URL || "http://localhost:8000"}/backup/download`} target="_blank">Backup</Button><Button component="label">Restore<input hidden type="file" accept=".db,.sqlite" onChange={restoreBackup} /></Button></Box></Box>
    ...
    <Box sx={{ display: "flex", justifyContent: "space-between", mt: 4, mb: 2 }}><Typography variant="h6">Sales history</Typography><Box sx={{ display: "flex", gap: 1 }}><Button onClick={exportSales}>Export CSV</Button><Button onClick={() => window.print()}>Print</Button><Button variant="contained" onClick={() => { setSaleLines([]); setAmountPaid(""); setSaleOpen(true); }}>Record sale</Button></Box></Box>
