import { Alert, Autocomplete, Box, Button, Card, CardContent, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, InputLabel, MenuItem, Paper, Select, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography } from "@mui/material";
...
export const CustomersSales = () => {
  ...
  const addSaleLine = () => {
    ...
    if (!item || item.unit_price <= 0 || !Number.isFinite(quantity) || quantity <= 0 || quantity > item.qty || !Number(salePrice) || Number(salePrice) <= 0 || saleLines.some((line) => line.stock_item_id === saleRecipe)) { setSaleError("Choose a priced item, enter a valid quantity within available stock, and enter a price greater than zero."); return; }
    setSaleLines([...saleLines, { stock_item_id: saleRecipe, quantity, unit_price: salePrice }]);
    setSaleRecipe(0); setSaleQty(""); setSalePrice(""); setSaleError("");
  };
  const saveSale = async () => {
    ...
    try {
      ...
      setSaleOpen(false); setSaleLines([]); setSaleReference(""); setSaleDueDate(""); setAmountPaid(""); setPaymentReference(""); setPaymentMethod("CASH"); setCustomerId(0); setSaleError("");
