...
export const Production = () => {
  ...
  const submit = async () => {
    ...
    try {
      const batch = await produce.mutateAsync({ recipe_id: recipeId, produced_qty: quantity, costing_method: method, sell_price_per_kg: overridePrice ? Number(overridePrice) : undefined });
      setSuccess(`Batch #${batch.id} recorded successfully.`); close(); setProducedQty(""); setBatchCount("1"); setOverridePrice("");
    } catch (requestError: any) { setError(requestError.response?.data?.detail || "Could not record the batch."); }
  };

  return <Box>
    <Box sx={{ display: "flex", alignItems: { xs: "stretch", sm: "center" }, flexDirection: { xs: "column", sm: "row" }, justifyContent: "space-between", gap: 2, mb: 3 }}>
      <Box><Typography variant="h4" sx={{ fontWeight: 800 }}>Production</Typography><Typography color="text.secondary">Record batches and review your recent output.</Typography></Box>
      <Button startIcon={<AddRoundedIcon />} variant="contained" size="large" onClick={() => setOpen(true)} disabled={!recipes.length}>Record batch</Button>
    </Box>
    <Card sx={{ mb: 3, background: "linear-gradient(135deg, #0f766e, #0891b2)", color: "white" }}><CardContent><Typography variant="overline" sx={{ opacity: .8 }}>Production history</Typography><Typography variant="h5" sx={{ fontWeight: 750 }}>{batches.length} batches recorded</Typography></CardContent></Card>
    <TableContainer component={Paper} sx={{ borderRadius: 3, overflowX: "auto" }}>
      <Table sx={{ minWidth: 680 }}><TableHead><TableRow><TableCell>Batch</TableCell><TableCell>Recipe</TableCell><TableCell>Date</TableCell><TableCell>Qty</TableCell><TableCell>Method</TableCell><TableCell>Cost</TableCell><TableCell>Revenue</TableCell><TableCell>Profit</TableCell></TableRow></TableHead><TableBody>
        {batchesLoading ? <TableRow><TableCell colSpan={8} align="center"><CircularProgress size={24} /></TableCell></TableRow> : batches.length ? batches.map((batch: any) => <TableRow key={batch.id} hover onClick={() => setDetailId(batch.id)} sx={{ cursor: "pointer" }}><TableCell>#{batch.id}</TableCell><TableCell sx={{ fontWeight: 650 }}>{batch.recipe_name}</TableCell><TableCell>{new Date(batch.produced_at).toLocaleDateString()}</TableCell><TableCell>{batch.produced_qty}</TableCell><TableCell>{batch.costing_method}</TableCell><TableCell>₹{batch.total_cost.toFixed(2)}</TableCell><TableCell>{batch.total_revenue == null ? "—" : `₹${batch.total_revenue.toFixed(2)}`}</TableCell><TableCell sx={{ color: (batch.profit ?? 0) >= 0 ? "success.main" : "error.main", fontWeight: 700 }}>{batch.profit == null ? "—" : `₹${batch.profit.toFixed(2)}`}</TableCell></TableRow>) : <TableRow><TableCell colSpan={8} align="center">No production batches recorded yet.</TableCell></TableRow>}
      </TableBody></Table>
    </TableContainer>
    <Dialog open={open} onClose={close} maxWidth="sm" fullWidth><DialogTitle sx={{ fontWeight: 750 }}>Record a batch</DialogTitle><DialogContent>
      ...
      <TextField margin="dense" label="Selling price per kg (optional)" type="number" fullWidth value={overridePrice} onChange={(event) => setOverridePrice(event.target.value)} />
